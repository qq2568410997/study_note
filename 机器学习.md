...
```
### 分布式TensorFlow
1. 分布式TensorFlow是由高性能的gRPC框架作为底层技术支持的
	+ gRPC是一个通信框架(google remote procedure call)
	+ 是一个高性能的、跨平台的RPC框架
		1. RPC协议----即远程过程调用协议
		2. 是指通过网络从远程计算机程序上请求服务
...
3. RPC框架  远程过程调用
	...
	+ 使用场景
		...
		2. 分布式系统
4. 单机多卡与多机多卡
	+ 单机多卡(这里的卡主要是指计算设备，如GPU)
		1. 概念: 单台服务器上有多块GPU设备
		2. 假设一台机器上有4块GPU，单机多GPU的训练过程如下
			* 在单机单GPU的训练中，数据是一个batch一个batch地训练
			* 在单机多GPU中，数据是一次处理4个batch
			* 变量或者说是参数，保存在CPU上
				1. 数据由CPU分发给4个GPU
				2. 在GPU上完成计算，得到每批次要更新的梯度
				3. 在CPU上收集4个GPU上要更新的梯度，计算一下平均梯度，然后更新
				4. 循环上述步骤
	+ 多机多卡(分布式)
		1. 概念: 分布式指的是多台计算机，充分使用多台计算机的性能，处理数据的能力
		2. 可以根据不同计算机划分不同的工作节点
		3. 当数据量或者计算量达到一台计算机处理能力的上限时，就必须使用分布式
			
			
5. TensorFlow分布式架构
	+ 集群
		1. 参数作业  parameter job
		2. 工作作业  worker job
	+ 参数作业所在的服务器称为是参数服务器，负责管理参数的存储和更新
	+ 工作节点的服务器主要从事计算的任务
		1. 运行op操作
	+ worker节点中需要一个主节点来进行会话的初始化，创建文件等操作，其他节点等待进行计算
		1. 主节点的作用主要是实现同步，管理其他节点计算时所需的资源
	+ 分布式更新参数的模式(协调存储，更新参数)
		1. 同步模式
			* ps等待所有worker都完成计算后得到的梯度变化量
			* 对所有得到的变化量求均值计算新的参数
			* 再将新参数更新传递给worker服务器进行下一次计算
			* 同步的意义在于: 期间可能有的worker计算快，此时就需要等待其他worker也计算结束
		2. 异步模式
			* 哪个workor计算结束，ps都将先对其进行新参数的更新
				1. worker1先结束  w1 = w - p1
				2. worker2后结束  w2 = w - p2
			* 直到所有节点都计算结束，此时才更新全局的参数(原始参数-所有节点在全部结束时的参数变化量取均值)
				1. w_ = w-(p1+p2)/2
	+ 分布式API
		1. 创建一个tf.train.ClusterSpec({"ps":ps_spec, "worker": worker_spec})
			* 用于对集群中的所有任务进行描述
			* 该描述内容对所有任务都应该是相同的
			* ps_spec ps服务器列表
			* worker_spec  worker服务器列表
		2. 创建一个tf.train.Server(server_or_cluster_def,job_name=None,task_index=None,protocol=None,config=None,start=True)
			* 用于创建一个任务(ps、worker),并运行相应作业上的任务
			...
			* task_index   任务索引
			* attibute  target
				1. 返回tf.Session()连接到此服务器的目标
			* method  join()
				1. 参数服务器端，等待worker服务器传递参数
		3. worker服务器使用指定的设备 tf.train.device("/job:worker/task:0/cpu:0")
			* 指定主节点设备  with tf.train.device(function)
				1. function: tf.train.replica_device_setter(worker_device="设备名",cluster=cluster)
					+ 通过此函数协调不同设备上的初始化操作
					+ worker_device 指定设备
					+ cluster  集群描述对象
			* tf.train.device(device_name)  使不同工作节点工作在不同的设备上，各自初始化
		4. 分布式会话API tf.train.MonitoredTrainingSession(master=None,is_chief=True,checkpoint_dir=None,hooks=None,save_checkpoint_secs=600,save_sunmarires_step=USE_DEFAULT,save_summaries_secs=USE_DEFAULT,config=None)
			* master  指定运行会话协议的ip和port(用于分布式)---即主节点worker的服务器访问地址
				1. "grpc://192.168.0.1:2000"
			* is_chief 是否为主worker节点(用于分布式)
				1. 如果为True，它将负责会话初始化和恢复基础的TensorFlow会话
				2. 如果为False，它将等待一位负责人初始化或者恢复TensorFlow会话
			* checkpoint_dir  检查点文件目录，events目录
			* config   会话运行的配置项
				1. tf.ConfigProto(log_device_placement=True)
			* hooks   可选的SessionRunHook对象列表
				1. 常用的钩子
					```
					[tf.train.StopAtStepHook(last_step=5000),tf.train.NanTensorHook(loss),_LoggerHook()]
						+ tf.train.StopAtStepHook(last_step=5000) 指定训练的次数，超过就会发生异常
						+ tf.train.NanTensorHook(loss)  判断指定的Tensor是否为NaN，为NaN则结束
					```
				2. 在使用钩子时需要定义一个全局步数----统计训练的总步数
					+ tf.contrib.framework.get_or_create_global_step()
			* should_stop()  是否异常停止
			* run()   跟session一样可以运行op
6. TensorFlow上设备的命名规则
	+ /job:ps/task:0/cpu:0 表示的是ps服务器第一台的第一个cpu设备


7. TensorFlow分布式集群的大致流程
	+ 对集群当中的一些ps、worker进行指定
	+ 创建对应的服务 ps和worker
		1. ps       join()
		2. worker   计算模型
		3. 指定默认的worker作为主节点   初始化会话、创建文件等
	+ 对worker指定其要使用的设备  with tf.train.device(设备名)
	+ 分布式会话

8. TensorFlow中tf.app.run() 默认运行的是main(argv)方法----看源码一目了然
9. 分布式案例
```
import tensorflow as tf
...
if __name__ == "__main__":
    ### tf.app下的run()函数默认是会运行main方法的，参数为argv
    tf.app.run()
```
### 阿里云服务器
**修改完实例登陆密码后需要重启**
...
公网IP： 47.110.49.217
私网IP:  172.16.23.48
root WBJ2568410997@Qq   908022
```
2. 131***0265
```
公网IP： 47.102.151.73
私有IP： 172.19.2.18
root wbj2568410997@qQ 
```
